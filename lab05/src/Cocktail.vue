    ...
    <div>
      ...
      <ul>
        <li v-for="ingredient in cocktail.ingredients">
          ...
          <button v-if="ingredient.price" @click="$emit('ingredientClick', ingredient)">Buy for CHF {{ingredient.price}}</button>
        </li>
      </ul>
    ...
    cocktail: {
      ...
      validator(value) {
        ...
        if (value.ingredients && value.ingredients.length) {
          return value.ingredients.every(ingredient => ingredient.quantity && ingredient.title);
        }
